...
class DocumentSerializer(serializers.ModelSerializer):
    ...
    def get_annotations(self, instance):
        ...
        # print (">>>HERE", self.context.get("request").user.id)
        project = instance.project
        model = project.get_annotation_class()
        serializer = project.get_annotation_serializer()
        annotations = model.objects.filter(document=instance.id)
        # project.collaborative_annotation = True
        # print(">>>>>>>>>>>>>>>>>>>", project.collaborative_annotation)
        ...
        # annotations = annotations.filter(RoleMapping.objects.values("role_id__name"))
        if request and request.user.id != 1:
            annotations = annotations.filter(Q(user=request.user) | Q(user=1))
